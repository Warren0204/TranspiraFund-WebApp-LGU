...
import {
    Trash2, CheckCircle, Plus, Users, X, Send, AlertCircle, Search, RefreshCw, Shield, Mail
} from 'lucide-react';
...
const useAccountLogic = () => {
    ...
    // 2. Real-Time Accounts State
    const [accounts, setAccounts] = useState([]);
    const [searchTerm, setSearchTerm] = useState('');

    // 2.1 Sync with Firestore users
    useEffect(() => {
        const q = query(
            collection(db, "users"),
    ...
    // 2.3 Filtered Accounts (uses debounced value)
    ...
    const [successEmail, setSuccessEmail] = useState(''); // Persist email for success modal
    ...
    const handleGenerateCredentials = async () => {
        ...
        try {
            ...
            setIsSending(false);
            setSuccessEmail(confirmingAccount.email); // Save email before clearing form
    ...
    return {
        accounts, filteredAccounts, searchTerm, setSearchTerm, missingRoles,
                ...
                <div className="space-y-4">
                    <div>
                        ...
                        >
                            ...
                            {roles.map(role => (
                                <option key={role.type} value={role.type}>{role.label}</option>
                            ))}
                        </select>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                type="text" name="firstName" placeholder="e.g. Sarah" required
                        ...
                        <div>
                            ...
                            <input
                                type="text" name="lastName" placeholder="Connor" required
                    ...
                    <div>
                        ...
                        <input
                            type="email" name="email" placeholder="official@lgu.gov.ph" required
                    ...
                    <button
                        onClick={onSubmit}
                        disabled={!form.roleType || !form.firstName || !form.lastName || !form.email}
...
const ConfirmAssignmentModal = memo(({ isOpen, data, onConfirm, onCancel, isProcessing, error }) => {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-[24px] shadow-2xl w-full max-w-lg transform transition-all animate-in zoom-in-95 duration-200 overflow-hidden">
                <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                    <h3 className="font-bold text-slate-900 text-lg">Confirm Assignment</h3>
                ...
                <div className="p-8">
                    ...
                    <button onClick={onConfirm} disabled={isProcessing} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 rounded-xl shadow-lg transition-all flex justify-center gap-2">
                        {isProcessing ? 'Processing...' : 'Generate & Send Credentials'}
                    </button>
                </div>
            </div>
        </div>
