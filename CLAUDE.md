...
**PE deletion side-effect.** When HCSD deletes a Project Engineer via `deleteOfficialAccount`, every active (non-Completed) project assigned to them is auto-unassigned in the same call: `projectEngineer` is cleared to `""`, `status` reverts to `"Delayed"`, and `engineerUnassignedAt` / `engineerUnassignedReason` are stamped. Completed projects keep the deleted engineer's UID as a historical "who did this work" record. The `ACCOUNT_DELETED` audit entry includes `unassignedActiveProjects` for forensic traceability, and the callable returns the count so the Staff Management page can surface a "N project(s) need a new engineer" toast.

**Engineer reassignment.** The Project Detail page's "Project Engineer" cell renders an amber dropdown of available PROJ_ENG users in the tenant whenever `projectEngineer` is empty (orphaned by a PE deletion, or never assigned at creation). Picking a replacement calls the `reassignProjectEngineer` callable, which validates the new PE is in the caller's tenant and has `role === "PROJ_ENG"`, sets `projectEngineer` to the new UID, flips `status` to `"Ongoing"`, stamps `engineerReassignedAt`, deletes the `engineerUnassignedAt` / `engineerUnassignedReason` fields, writes a `PROJECT_REASSIGNED` HCSD audit row, and fires a `PROJECT_ASSIGNED` notification to the new engineer (same notification shape as project creation, so mobile renders it identically). Completed projects are rejected with `failed-precondition` — once a project is done, its historical engineer must not be rewritten.
...
**Slippage-triggered Delayed.** The `detectProjectSlippage` scheduled function runs daily at 08:00 Asia/Manila and scans every non-Completed project that has an assigned engineer. For each, it computes slippage (`timeElapsed% − actualPercent%`; same formula as `client/src/utils/slippage.js`) and: (a) on the on-schedule → slipping transition, persists `status="Delayed"`, stamps `slippageDetectedAt` + `slippagePercent`, and fans out a `category: "system"` `PROJECT_DELAYED` notification with title `"Project is now Delayed"` (severity `warn`) to BOTH the HCSD creator and the assigned PE; (b) while still slipping each subsequent morning, fans out a `"Project still Delayed"` reminder notification (`metadata.reminder = true`) and refreshes `slippagePercent` — daily nag until resolved; (c) on the slipping → recovered transition, clears the slippage fields and restores `status="Ongoing"` if no other delay reason lingers (e.g. `engineerUnassignedAt`) — no "recovered" notification is sent, just silent restoration. The dashboard donut counts Ongoing-with-positive-slippage as Delayed at render time too, so the count stays correct in the 24h gap before the next cron run. Manual one-shot via the `runSlippageScanNow` callable (HCSD or MIS) or `scripts/trigger-slippage-scan.js --apply`.
...
| `PROJECT_REASSIGNED` | `reassignProjectEngineer` Cloud Function — HCSD picks a replacement PE from the Project Detail dropdown (for orphaned projects after a PE deletion, or routine reassignment) | Project name | Project |
